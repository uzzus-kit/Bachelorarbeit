...
exclusion_period <- seq(from = as.Date("2019-06-30"), to = as.Date("2023-07-03"), by = 1)               #Hier wird die Covid Zeit ausgeschlossen(Nicht sonderlich relevant für BA)
...
forecast_dates <- seq(from = as.Date("2024-07-04"),                                                   #Zeitpunnkt für Nowcast festlegen
...
n_history_dispersion <- 15                                                                            #Einstellungen der Meta Daten bis hier her kann man es lassen
n_history_expectations <- 15
max_delay <- 4
max_horizon <- 4 # ifelse(discard_last_value, 5, 4)
quantile_levels_nowcast <- seq(from = 0.025, to = 0.975, by = 0.05)                                   #Auch Quantil Level können gleich bleiben.
...
for(data_source in data_sources){                                                                     #Beginn relevanter Code
  ...
  # run over forecast dates to generate forecasts:                                                    #Läuft über Daten für die Nowcast erstellt werden sollen
  for(i in seq_along(forecast_dates)){
    ...
    for (disease in diseases) {
      ...
      for(ag in ags){                                                                               #Primär keine Stratifizierung über Daten
        # prepare for plotting:
        # truth data as of forecast_date, subset to relevant stratum
        reporting_triangle_back_in_time <- data_as_of(dat_truth = triangles[[disease]], date = forecast_date,   #Funktion Erzeugt für jede Woche die Datensätze  die zu dem jewiligen Zeitpunkt zur verfügung Standen, also nur die gemeldeten Fälle bis zu der Woche ohne Korrekturen in späteren Wochen. Also quasi die Rückrechnung der tatsächlichen Nachträglichen Anpassung der Werte.
                                                      location = "DE", age_group = ag, max_lag = max_delay)
        
        
        # current version of truth data, subset to relevant stratum                                 #2.Datensatz, der nur zum Plotten verwendet wird
        reporting_triangle_current <- data_as_of(dat_truth = triangles[[disease]], date = Sys.Date(),
                                                 location = "DE", age_group = ag, max_lag = max_delay)
        
        # compute nowcast:                                                                          #Tatsächliche Durchführung des Nowcasts
        nc <- compute_nowcast(observed = triangles[[disease]],                                      #Observed ist rsv, Covid oder Influenza Datensatz
                              location = "DE",
                              age_group = ag,
                              observed2 = triangles[[disease]],                                     #Observed 2 selber rein Basteln=gesamte Daten triangle. Wichtig da Daten erst seit letzem Herbst existieren. Daten triangle für die gesamte Daten verwenden unter der Annahme, dass die Meldeverzüge für die einzelne Krankheit gleich den der gesamt Krankheiten sind.
                              location2 = "DE",
                              age_group2 = "00+",
      ...
      ############################################################                              #Ab hier Nowcast, sollte so bleiben können
      ...
      # replace last weeks with nowcasts and generate forecasts:                                #Für jedes Quantil Level wird ein Forecast gemacht. Bis 221 Nur Datenvorverarbeitung
      for (j in seq_along(quantile_levels_nowcast)) {
        ...
        # control list for hhh4                                                                 #Ab hier tatsächliches hhh4 Forecasting
        ctrl <- list(end = # list(f = addSeason2formula(~0 + fe(1, unitSpecific = TRUE),
                       #                             period = 52.25)),
                       list(f = ~ 0),
