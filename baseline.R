...
compute_expectations <- function(observed, observed2 = NULL, borrow_delays = FALSE, n_history = 20, remove_observed = TRUE){
  ...
  if(nrow(observed2) < n_history){
    print(n_history)
    print(nrow(observed2))
    print(observed2)
    stop(paste0("n_history=",n_history,"  ","nrow=",nrow(observed2),"  Available reporting triangles are too short for the chosen n_history."))
  }
...
fit_dispersion <- function(observed, location, age_group,
  ...
  to_keep <- abs(expectation_to_add_already_observed[, 1]) >= 0.1             #Filtert alles raus was =0 ist
  ...
  print("Debug")
  print(disease)
  print(to_add_already_observed)
  print(nrow(to_add_already_observed))
...
compute_nowcast <- function(observed, location = "DE", age_group = "00+",
                            ...
                            observed2 = NULL, location2 = NULL, age_group2 = NULL,
                            weekday_data_updates = "Thursday",
                            max_delay = 5, n_history_expectations = 15, n_history_dispersion = 15, 
                            quantile_levels = c(0.025, 0.1, 0.25, 0.5, 0.75, 0.9, 0.975)){
  
  # check reporting triangle and forecast_date:
  if(forecast_date > max(as.Date(observed$date)) + 7){
