...
preprocess_reporting_triangle <- function(df, max_delay = 4) {        #Funktion, die 2 Argumente, einen Data Frame und den standard Wert 4 erwartet.
  
  # restrict to first 5 columns.
  col_4w <- which(colnames(df) == paste0("value_", max_delay, "w"))   #Legt spaltennamen fest
  df <- df[, 1:col_4w]                                                #Speichert in df die erste Spalte von dem Eingabe Daten Frame
  ...
  # Loop over each row
  for (i in 1:nrow(df)) {
    to_subtract <- 0                                                  #Variable Wert 0 zuweisen
    row <- df[i, ]                                                    #Vektor row die erste Zeile vom Daten Frame zuweisen
    
    # Loop over the columns starting from the last column back to column 5      Wann wird to_subtract geändert?
    for (j in rev(value_cols)) {                                      #Ausgleich von Negativen Werten
      value <- row[[j]]
