...
triangles[[diseases[5]]]=read.csv("C:\\Users\\felix\\Desktop\\Uni\\BA\\Daten\\reporting_triangle-icosari-Rest.csv",
                                  colClasses = c("date" = "Date"), check.names = FALSE)
triangles[["Durchschnitt"]]=read.csv("C:\\Users\\felix\\Desktop\\Uni\\BA\\Daten\\reporting_triangle-icosari-Durchschnitt.csv",
                                   colClasses = c("date" = "Date"), check.names = FALSE)
...
targets[[diseases[5]]]=read.csv("C:\\Users\\felix\\Desktop\\Uni\\BA\\Daten\\target-icosari-rest.csv",
                                colClasses = c("date" = "Date"), check.names = FALSE)
targets[["Durchschnitt"]]=read.csv("C:\\Users\\felix\\Desktop\\Uni\\BA\\Daten\\target-icosari-Durchschnitt.csv",
                                 colClasses = c("date" = "Date"), check.names = FALSE)
...
for(i in seq_along(forecast_dates)){                    #Durchläuft alle Prognosedaten der Liste forecast_dates
  ...
  for (disease in diseases[[j]]) {                           #Durchlaufe Sari, Covid, Rsv und Influenza Daten
    ...
    nc <- compute_nowcast(observed = triangles[[disease]], # this is the reporting triangle for which to compute nowcasts
                          ...
                          observed2 = triangles$Durchschnitt, # use complete sari to estimate delays
                          location2 = "DE",
                          age_group2 = "00+",
                          type = "additions",
...
write.csv(do.call(rbind,SARI_liste),"C:\\Users\\felix\\Desktop\\Uni\\BA\\Daten\\Nowcast_gesamt_Durschnitt.csv",row.names = FALSE)
Nowcast=read.csv("C:\\Users\\felix\\Desktop\\Uni\\BA\\Daten\\Nowcast_gesamt_Durschnitt.csv")
Nowcast_einzel=list()
i=1
for (disease in diseases){
