...
diseases <- c("sari", "sari_covid", "sari_influenza","Rest", "sari_rsv")
...
#Rest berechnen
  ...
  for(i in 1:nrow(sari_gekürzt)){
    match_row <- triangles[[diseases[1]]][
      triangles[[diseases[1]]]$date == sari_gekürzt[i, 5] & 
        triangles[[diseases[1]]]$age_group == sari_gekürzt[i, 2], 
    ]
    
    if (nrow(match_row) == 1) {
      sari_gekürzt[i, ] <- match_row
    } else {
      warning(paste("Kein eindeutiger Match in Zeile", i))
    }
  }
